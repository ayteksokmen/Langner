package com.ayteksokmen.langner.Chat.core.users.get.all;

import android.text.TextUtils;
import android.util.Log;

import com.ayteksokmen.langner.Chat.models.Chat;
import com.ayteksokmen.langner.Chat.models.User;
...
public class GetUsersInteractor implements GetUsersContract.Interactor {
		...
		FirebaseDatabase.getInstance().getReference().child((Constants.ARG_CHAT_ROOMS)).addListenerForSingleValueEvent(new ValueEventListener() {
			...
			public void onDataChange(DataSnapshot dataSnapshot) {
				...
				while (dataSnapshots.hasNext()) {
					...
					String chatRoomTitle = dataSnapshotChild.getKey();
					String sender = chatRoomTitle.split("_")[0];
					String receiver = chatRoomTitle.split("_")[1];
					if (sender.equals(FirebaseAuth.getInstance().getCurrentUser().getUid())) {
						getChatUsers(receiver);
					}
					else Log.d("HOOOOP","HİÇ ARKADAŞIN YOK EZİK !");
				}
			}
	...
	public void getChatUsers(final String receiverUid){
		FirebaseDatabase.getInstance().getReference().child(Constants.ARG_USERS).addListenerForSingleValueEvent(new ValueEventListener() {
			...
			public void onDataChange(DataSnapshot dataSnapshot) {
				...
				while (dataSnapshots.hasNext()) {
					...
					if (!TextUtils.equals(receiverUid, FirebaseAuth.getInstance().getCurrentUser().getUid()) && !TextUtils.equals(user.uid, FirebaseAuth.getInstance().getCurrentUser().getUid())) {
						users.add(user);
					}
					else Log.d("HOOOOP","HİÇ ARKADAŞIN YOK EZİK !");
				}
